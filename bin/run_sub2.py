...
database = r"C:\Users\caleb\code\code\HITS\Ensnare\bin\temp.db"
...
def runSub(depth:int, structure:list, excc_extensions:list, folder:str = None, runCollectors=True, startTime=None, baseDir=None):
    ...
    collected_filename = collected_filename.replace(os.sep, "__") #-2 -> take off the collector from the path
...
def send_emails(time):
    ...
    sql = "select email, count(*) from tblNotify where batchTime in (select batchTime from tblNotify group by batchTime order by batchTime limit 1) group by email"
    toNotify = session.query(Notify).all()
    emailDict = sqlGetJsonFromQuery(sql, conn)
    for row in emailDict:
        to = row["email"]
        sql = f"select * from tblNotify where email = '{to}'"
        toNotify = sqlGetJsonFromQuery(sql, conn)
        for x in toNotify:
            if str(x["batchTime"]) == str(time):
